...
var slickslider = {
  init: function(){
    ...
    $('.js-slick-slider').each(function (index, value){
      var $homepageSlider = $(this);
      var settings = {
        slidesToShow:parseInt($homepageSlider.attr('data-slide-to-show')),
        adaptiveHeight:(($homepageSlider.attr('data-adaptive-height') == "true") ? true : false),
        slidesToScroll:parseInt($homepageSlider.attr('data-slide-to-scroll')),
        infinite: true,        
        autoplay: (($homepageSlider.attr('data-auto-play') == "true") ? true : false),
        autoplaySpeed:parseInt($homepageSlider.attr('data-slide-speed'))*1000,
      }
      console.log(settings)
      if($homepageSlider.hasClass('slick-initialized')){
        $homepageSlider.slick('unslick').slick(settings);
      }
      else{
        $homepageSlider.slick(settings);
      }
...
var blogslider = {
  init: function(){
    ...
    $('.js-blog-slider').each(function (index, value){
      var $homepageSlider = $(this);
      var settings = {
        slidesToShow:parseInt($homepageSlider.attr('data-slide-to-show')),
        adaptiveHeight:(($homepageSlider.attr('data-adaptive-height') == "true") ? true : false),
...
// //    var ele = $('.your-class');   
// //    var next_slide = $('#next-slide');
// //    var previous_slide = $('#previous-slide');

// //    var settings = {
// //      slidesToShow:parseInt( ele.attr('data-slide-to-show')),
// //      adaptiveHeight:ele.attr('data-adaptive-height'),
// //      slidesToScroll	:parseInt(ele.attr('data-slide-to-scroll')),
// //          infinite: true
// //    }

//    console.log(settings);
// //    $('.your-class').slick(settings);

//    $("#next-slide").click(function(e) { // Added a '.'
//     $(".your-class").slick('slickNext'); // Switched to '.slick-slider'
// });
//    $("#previous-slide").click(function(e) { // Added a '.'
//     $(".your-class").slick('slickPrev'); // Switched to '.slick-slider'
//    });
...
const initSearchPane = () => {
  console.log('initSearchPane');
  const searchPaneTrigger = $('.header-search-bar');
  const searchPane = $('#search_popup');

  const searchForm = $('#header-search-form');
  const searchInput = searchForm.find('input[type="text"]');
  const searchClear = searchForm.find('#header-search-clear');
  const searchResults = $('#header-search-results');
  const searchUrl = 'https://sd-hirendev.myshopify.com/search';
  ...
  const searchReset = () => {
    searchInput.val('');
    searchResults.empty();
    html.removeClass('search_pane_has_value');

//     loadingBarTrigger('done');
  }

  const searchResultsTrigger = () => {
    ...
    searchInputThrottle = setTimeout(function () {
      ...
      if (term.length >= 3 &&
          term != searchInput.attr('data-term')) {
        // add active state
        html.addClass('search_pane_has_value');

        // register search term to avoid double triggering ajax
        searchInput.attr('data-term', term);

        // start loading state
//         loadingBarTrigger('start');

        // if abort current ajax request, if exists
        (productSearchRequest !== null)
        ? productSearchRequest.abort() : '';

        (articleSearchRequest !== null)
        ? articleSearchRequest.abort() : '';
        
        (pageSearchRequest !== null)
        ? pageSearchRequest.abort() : '';

        // trigger ajax request
        productSearchRequest = $.getJSON(
          `${searchUrl}?type=product&q=${term}&view=json`
        ).always(function (data) {
          searchProductResults = '';

          if (data.results_count > 0) {
            searchViewAllButton = (data.results_count > 2)
            ? true : false;

            searchProductResults += `<div id="search-product-list">
<div class="t-link cr-subdued">Products</div>`;

            $.each(data.results, function (i, item) {
              if (i < 2 && item.title.toLowerCase().indexOf("gift box") === -1) {
                searchProductResults += `<div 
class="search-product g-flex">
<a href="${item.url}" title="${item.title}"></a>
<div class="search-product-thumbnail thumbnail">
...
<div class="t-b-2">${item.title}</div>
<div class="t-b-2 cr-subdued">${item.price}</div>
</div>
</div>`;
              }
            });
        ...
        var searchArticleResults = '';
        articleSearchRequest = $.getJSON(
          `${searchUrl}?type=article&q=${term}&view=json`
        ).always(function (data) {
          if (data.results_count > 0) {
            searchViewAllButton = (searchViewAllButton == true ||
                                   data.results_count > 2)
            ? true : false;

            searchArticleResults += `<div id="search-article-list" class="t-b-2">
<div class="t-link cr-subdued">Blog Posts and Pages</div>
<ul class="t-b-2">`;

            $.each(data.results, function (i, item) {
              if (i < 2 && !item.url.includes('/pages/')) {
                searchArticleResults += `<li>
<a href="${item.url}">${item.title}</a>
        ...
        pageSearchRequest = $.getJSON(
          `${searchUrl}?type=page&q=${term}&view=json`
        ).always(function (data) {
          searchPageResults = '';
...
//             searchPageResults += `<div id="search-article-list" class="t-b-2">
// <div class="t-link cr-subdued">Blog Posts and Pages</div>
// <ul class="t-b-2">`;
...
// </div>`;
          ...
          searchArticleResults += `</ul></div>`;
        });

        $.when(
          productSearchRequest, articleSearchRequest, pageSearchRequest
        ).done(function () {
//           loadingBarTrigger('done');
          ...
          searchResults.html(`${(searchArticleResults == '' &&
                                 searchProductResults == '')
                             ? `<div id="search-no-results">
<div class="t-b-2 cr-gray-d">No results for your search term.</div>
</div>`
                             : `${searchProductResults}${searchArticleResults}${searchPageResults}${(searchViewAllButton == true)
                             ? `<div id="search-results-view-all">
<a 
class="btn-block" 
...
View All Results</a>
  ...
  searchPaneTrigger.click(function () {
    html.toggleClass('search_pane_active');

    setTimeout(function () {
      html.hasClass('search_pane_active')
      ? searchInput.focus()
      : '';
    }, 300);
  });
  ...
  _window.click(function (e) {
    ...
    if (html.hasClass('search_pane_active') &&
        !target.is('.header-search-bar') &&
        !target.closest('.header-search-bar').length &&
        !target.is('#search_popup') &&
        !target.closest('#search_popup').length) {

      html.removeClass('search_pane_active');
      setTimeout(searchReset, 300);
    }
  });

  searchInput.on('keyup change', searchResultsTrigger);

  searchForm.on('submit', function (e) {
    e.preventDefault();

    window.location.href = `${searchUrl}?q=${searchInput.val()}`;
  }).on('clearSearch', searchReset);
...
const initSearchIndex = () => {
    console.log('initSearchIndex');

  const searchHeaderInput = $('#search-header-input');
  const searchHeaderClear = $('#search-header-clear');

  searchHeaderInput.on('keyup change', function () {
    (searchHeaderInput.val())
    ? html.addClass('search_header_input_has_value')
    : html.removeClass('search_header_input_has_value')
  });

  searchHeaderClear.click(function () {
    searchHeaderInput.val('').focus();
  });

  const searchResultsTabButton = $('#search-results-tab button');

  searchResultsTabButton.click(function () {
    let _this = $(this);

    _this.addClass('active')
    .siblings().removeClass('active');

    _this.parent().attr('data-results-tab',
                        _this.attr('data-results-tab-trigger'));
  });
}